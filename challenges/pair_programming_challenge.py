...
def create_function():
	username = input("please enter username for this bank account: ")
	password = input("please enter password for this bank account: ")
	bank_account = {"username":username,"password":password, "balance":0.0}
	return bank_account
...
deposit(my_account,700)
deposit(my_account,800)
deposit(my_account,900)
...
def withdrawal(account,amount):
	if amount <= account['balance']:
		...
		print("there is not enough available to make that withdrawal.")


withdrawal(my_account,1000)
print(my_account)
withdrawal(my_account,300)
print(my_account)
withdrawal(my_account,200)
print(my_account)
withdrawal(my_account,600)
print(my_account)
withdrawal(my_account,2200)
print(my_account)
...
'''
# username = input("please verify username for this bank account: ")
# 	password = input("please verify password for this bank account: ")
...
def secure_deposit(account,amount):
	validate(account)
	account['balance'] += amount
	print("now depositing...")

def secure_withdraw(account,amount):
	validate(account)
	if amount <= account['balance']:
		account['balance'] -= amount
		print("currently withdrawing...")

	else: 
		print("there is not enough available to make that withdrawal.")

#test out secure functions
secure_deposit(my_account,700)
print(my_account)

secure_withdraw(my_account,600)
print(my_account)

def log_out(account):
	logged_out = input("is there anything else?: ")
	if logged_out in ['yes', 'y', 'si', 'YES', 'Y', 'SI', 'Yes', 'Si']:
		validate(account)
